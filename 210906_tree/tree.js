...
window.onload = function init()
{
	...
	var leaf = new Float32Array([
		-0.3, 0.7, -0.7, 0.3, 0.1, 0.3,   // triangle 
		]);
	...
	gl.uniform4fv(offsetLoc,[0,-0.4,0,0]); // color (R,G,B,A)	
	render(first, count) // render function

	gl.uniform4fv(offsetLoc,[0,-0.8,0,0]); // color (R,G,B,A)	
	render(first, count) // render function

    gl.uniform4fv(offsetLoc,[0.4,0.0,0,0]); // color (R,G,B,A)	
	render(first, count) // render function

    gl.uniform4fv(offsetLoc,[0.4,-0.4,0,0]); // color (R,G,B,A)	
	render(first, count) // render function

    gl.uniform4fv(offsetLoc,[0.4,-0.8,0,0]); // color (R,G,B,A)	
	render(first, count) // render function
	...
	var body = new Float32Array([
		-0.45, -0.5, -0.15, -0.5, -0.45, -1, // triangle
		-0.15, -0.5, -0.45, -1, -0.15, -1     // triangle
		]);
    ...
    gl.uniform4fv(offsetLoc,[0.4,0,0,0]); // color (R,G,B,A)
    render(first, count) // render function	
